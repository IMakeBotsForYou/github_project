...
def start_draw(s, to_highlight=None, reload_colors=False, erase=False, force=False, admin_view=False, repo=0):
    global all_data

    if admin_view:
        path = f'./static/admin_{repo}.png'
        bruh_repo = repo

    else:
        path = f'./static/rendered_{s["user"]}_0.png'
    ...
    if not db_obj:
        initialize_graphics()
    user_name = s['user']
    s['file_name'] = ("rendered_" if not admin_view else "admin_") + user_name
    # noinspection PyUnresolvedReferences
    commits = db_obj.get_commits_info(bruh_repo)
    ...
    if x == 0 or y == 0 or force:
        calculate_size(bruh_repo,
                       data=[[item[1], int(db_obj.get("commits", "depth", condition=f'id="{item[0]}"')[0])] for
                             item in commits])
    x, y = all_data['size']
